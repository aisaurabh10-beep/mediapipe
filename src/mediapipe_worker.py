...
def log_stderr(message):
    """Write messages to stderr for parent process logging."""
...
try:
    ...
    log_stderr("WORKER: MediaPipe imported")
    ...
    log_stderr("WORKER: FaceMesh ready")

    # 3D face model for solvePnP
    MP_LANDMARKS = {
        "nose": 1,
        "chin": 152,
        "left_eye": 33,
    ...
    def get_pose_and_alignment(image_rgb):
        ...
        try:
            ...
            log_stderr("WORKER: Landmark index error")
            return None, 0, 0, 0, 0.0
        ...
        if eye_to_eye_dist < 1e-6:
            aspect_ratio = 1.0
        else:
            aspect_ratio = forehead_to_chin_dist / eye_to_eye_dist

        # Calculate roll for alignment
        dx = right_eye[0] - left_eye[0]
        dy = right_eye[1] - left_eye[1]
        roll_angle = np.degrees(np.arctan2(dy, dx))

        # Try solvePnP for accurate pose
        ...
        # Align face by rotating based on eye angle
        eye_center = (
    ...
    # Main loop
    def main_loop():
        """Process faces from main process."""
        log_stderr("WORKER: Ready, waiting for data...")
        while True:
            try:
                ...
                log_stderr("WORKER: Pipe closed, exiting")
                break
            except Exception as e:
                try:
                    err_str = traceback.format_exc()
                    log_stderr(f"WORKER ERROR: {err_str}")
                    error_response = (None, 0.0, 0.0, 0.0, 0.0, err_str)
                    out_data = pickle.dumps(error_response)
                    write_message(out_data)
                except Exception:
                    log_stderr("WORKER: Failed to send error, exiting")
                    break
    ...
    log_stderr("WORKER FATAL ERROR during initialization")
